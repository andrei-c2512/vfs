...
pub fn test_file_ops_1() -> Result<(), Error> {
    write_test_header(8, "Testing having multiple directories AND small files");
    let mut vfs = Vfs::new("file_ops1.vfs")?;
    let vfs_files = ["rs/abc.txt", "rs/def.txt"];
    let os_files = ["res/test1.txt", "res/test2.txt"];
    ...
    {
        for i in 0..vfs_files.len() {
            let mut file = vfs.create(vfs_files[i])?;
            file.write_from_file(os_files[i])?;
        }
    }
...
pub fn test_file_ops_2() -> Result<(), Error> {
    write_test_header(
        10,
        "Testing having multiple directories AND big files + overwriting a file in the middle of 
                      the block device, to observe fragmented file correcteness",
    );
    let mut vfs = Vfs::new("file_ops2.vfs")?;
    let paths = [
        "etc",
        "etc/conf",
        "etc/tmp",
    ...
    let mut file = vfs.create("goon.txt")?;
    file.write_from_file("res/background.jpeg")?;

    let mut os_file = open_output_file("output/background_5.jpeg")?;
    vfs.copy_from_vfs("goon.txt", &mut os_file)?;

    Ok(())
}

fn size_test_12mb_file_ops_3() -> Result<(), Error> {
    write_test_header(12, "Testing moving a 12MB file on the vfs");
    let mut vfs = Vfs::new("large.vfs")?;
    let vfs_img_file = "img.bmp";
    vfs.copy_into_vfs("res/background.bmp", vfs_img_file)?;
