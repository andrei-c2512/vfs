...
impl Vfs {
    ...
    pub fn read_dir(&self, path: &str) -> Result<Vec<u32>, Error> {
        ...
        match node {
            Node::Directory(dir_data) => Ok(dir_data.children.clone()),
            Node::File(_) => Err(Error::BadCall(
                "Attempted to call read_dir on a file".to_string(),
            )),
        }
    }
    pub fn create_dir(&mut self, path: &str) -> Result<ops::Directory, Error> {
        ...
        let _dir_id = {
            ...
            let dir = DirectoryData::from(
                INode::from(name_id, 0, DateTime::now(), DateTime::now(), 0),
                Vec::new(),
            );
    ...
    pub fn open_file(&self, path: &str) -> Result<ops::File, Error> {
        ...
        match node {
            Node::Directory(_) => Err(Error::BadCall(
                "Attempted to call open_file on a directory".to_string(),
            )),
            Node::File(_) => Ok(ops::File::from(
                node_id,
                self.header.clone(),
                self.block_device.clone(),
                self.file.clone(),
    ...
    fn open_os_file(path: &str) -> Result<fs::File, Error> {
        let res = fs::OpenOptions::new().read(true).write(true).open(path);
        match res {
            Ok(file) => Ok(file),
            Err(err) => Err(Error::FileOps(err.to_string())),
        }
    ...
    fn update_os_file(&mut self) {
        ...
        data.extend_from_slice(&self.block_device.borrow().serialize());
        {
