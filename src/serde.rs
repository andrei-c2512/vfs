...
use crate::util::date_time::{Date, DateTime, Time};
...
impl Serde for DateTime {
    fn serialize(&self) -> Vec<u8> {
        let mut result = Vec::new();
        result.extend_from_slice(&self.date.serialize());
        result.extend_from_slice(&self.time.serialize());
