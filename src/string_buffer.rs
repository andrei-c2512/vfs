...
impl StringBuffer {
    ...
    pub fn get(&self, s: &str) -> Result<u32, Error> {
        match self.name_map.get(s) {
            Some(num) => Ok(*num),
            None => Err(Error::InvalidDirective(format!("Bad string: {} ", s))),
        }
    }
    ...
    fn deser_str(buffer: &[u8], length: usize) -> Result<String, Error> {
        ...
        if bytes_opt.is_none() {
            return Err(Error::InvalidStringBuffer(
                "Did no provide the correct number of bytes for the string".to_string(),
            ));
        }
        match String::from_utf8(bytes_opt.unwrap().to_vec()) {
            Ok(res) => Ok(res),
            Err(err) => Err(Error::InvalidStringBuffer(err.to_string())),
        }
    }
}
...
impl Serde for StringBuffer {
    fn serialize(&self) -> Vec<u8> {
        // --- REWRITE: too many string copies
        let mut buffer =
            Vec::with_capacity(self.serialization_size() + STRING_BUFFER_PREAMBLE.len());
        buffer.extend_from_slice(STRING_BUFFER_PREAMBLE.as_bytes());
        let sorted_map = self.map_as_vec();

        // write down the size
        buffer.extend_from_slice(&(sorted_map.len() as u32).to_be_bytes());
        for item in sorted_map.iter() {
            buffer.extend_from_slice(&(item.len() as u32).to_be_bytes());
    ...
    fn deserialize(buffer: &mut &[u8]) -> Result<Self, Error> {
        ...
        if !buffer.starts_with(STRING_BUFFER_PREAMBLE.as_bytes()) {
            return Err(Error::InvalidPreamble(
                "Did not find the preamble specific to the string buffer".to_string(),
            ));
        }
